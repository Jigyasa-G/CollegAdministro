...
def hel():
    ...
    conn.execute('CREATE TABLE IF NOT EXISTS users (name TEXT, usn TEXT UNIQUE,email TEXT UNIQUE, pass TEXT)')
   ...
   if request.method == 'POST':
      try:
         ...
         with sqlite3.connect("database1.db") as con:
             cur = con.cursor()
             cur.execute("INSERT INTO users (name,usn,email,pass) VALUES (?,?,?,?)",(nm,usn,email,passs))
             con.commit()
...
conn.execute("CREATE TABLE IF NOT EXISTS studentreq(usn TEXT,document TEXT,description TEXT)")
...
def lists():
    ...
    cur.execute("select * from studentreq")
...
def addre():
    if request.method == 'POST':
        try:
            ...
            with sqlite3.connect("notifications.db") as con:
                
                cur=con.cursor()
                
                cur.execute("INSERT INTO studentreq(usn,document,description) VALUES(?,?,?)",(usn,doc,desc))
